...
enumerate block = B.enumerate ø !$? block
exercice block = BI.parEnvironmentPar "exo" [] !$? block
pW x = B.para !$? x
minipage x y = B.minipage B.normal x !$? y

itemW :: (MonadWriter (f ListItem) m, Monad f) => ParItemW -> m ()
itemW = tell . return . B.item . execWriter
